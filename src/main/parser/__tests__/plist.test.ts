...
describe("parseInfoPlist", () => {
	...
	it("should extract privacy usage description strings", () => {
		...
		expect(result.data.privacyUsageStrings).toEqual({
			...
			NSLocationWhenInUseUsageDescription: "We need location for nearby features"
		});
	});
	...
	it("should return an error for malformed plist", () => {
		const appDir = makeTmpAppBundle();
		fs.writeFileSync(path.join(appDir, "Info.plist"), "this is not a valid plist at all");
...
describe("parseMobileprovision", () => {
	it("should extract XML from a DER-like envelope and parse fields", () => {
		...
		const prefix = Buffer.from([
			0x30, 0x82, 0x10, 0x00, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x07, 0x02
		]);
		const xmlBuf = Buffer.from(MOBILEPROVISION_XML, "utf-8");
		const suffix = Buffer.from([0x00, 0x00, 0x00, 0xff, 0xfe, 0xab, 0xcd, 0xef]);
		...
		expect(d.ProvisionedDevices).toEqual(["aabbccdd11223344", "eeff00112233aabb"]);
	...
	it("should return an error when no XML boundaries are found", () => {
		...
		const garbage = Buffer.from([0x30, 0x82, 0x00, 0x50, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff]);
