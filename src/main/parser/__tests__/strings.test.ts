...
import { extractStrings, type StringEntry } from "../strings";

// ── Fixture Helpers ──────────────────────────────────────────────────

/** Create a minimal Section64 with sensible defaults. */
function makeSection(segname: string, sectname: string, offset: number, size: number): Section64 {
	return {
		sectname,
		segname,
...
describe("strings", () => {
	...
	describe("__cstring extraction", () => {
		it("extracts null-terminated strings >= 4 chars", () => {
			...
			const seg = makeSegment("__TEXT", 0x100000000n, BigInt(bufSize), 0n, BigInt(bufSize), [
				sect
			]);
	...
	describe("__cfstring resolution with rebaseMap", () => {
		it("resolves a CFString struct via chained fixup rebase", () => {
			...
			const dataSeg = makeSegment("__DATA", 0x100000100n, 256n, 256n, 256n, [
				cfstringSection
			]);
