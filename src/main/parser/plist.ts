 ...
 */
function extractPrivacyStrings(
  raw: Record<string, unknown>
...
export function parseInfoPlist(
...
): ParseResult<InfoPlistData> | null {
  ...
  try {
    ...
    const data: InfoPlistData = {
      ...
      CFBundleShortVersionString: raw.CFBundleShortVersionString as
        | string
        | undefined,
      ...
      UIRequiredDeviceCapabilities: raw.UIRequiredDeviceCapabilities as
        | string[]
        | undefined,
  ...
  try {
    ...
    if (xmlStart === -1 || xmlEnd === -1) {
      return {
        ok: false,
        error:
          "Failed to parse mobileprovision: could not find XML plist boundaries",
      };
    }
    ...
    const data: MobileprovisionData = {
      ...
      Entitlements: parsed.Entitlements as
        | Record<string, unknown>
        | undefined,
