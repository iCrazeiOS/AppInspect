 ...
 */
function parseFixupsHeader(view: DataView, offset: number, le: boolean): ChainedFixupsHeader {
	return {
 ...
 */
function parseStartsInSegment(view: DataView, offset: number, le: boolean): ChainedStartsInSegment {
...
function walkChain(
	...
	while (iterations < maxIterations) {
		...
		if (bind === 0n) {
			...
			const symbolName =
				ordinal < imports.length
					? imports[ordinal]!.symbolName
					: `<unknown_ordinal_${ordinal}>`;
	...
	while (iterations < maxIterations) {
		...
		if (bind === 0n) {
			...
			if (auth === 1n) {
				...
				const addend = (addendRaw & 0x40000n) !== 0n ? addendRaw - 0x80000n : addendRaw;
				const symbolName =
					ordinal < imports.length
						? imports[ordinal]!.symbolName
						: `<unknown_ordinal_${ordinal}>`;
...
export function buildFixupMap(
	...
	for (let segIdx = 0; segIdx < segCount; segIdx++) {
		...
		for (let pageIdx = 0; pageIdx < startsInSeg.page_count; pageIdx++) {
			...
			const firstFixupOffset = segFileOffset + pageIdx * startsInSeg.page_size + pageStart;
