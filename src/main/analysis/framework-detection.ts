...
export function detectAppFrameworks(appBundlePath: string, linkedLibs: string[] = []): string[] {
	...
	const hasAnyFramework = (...names: string[]): boolean => names.some(hasFramework);
	...
	// Unreal Engine
	if (
		hasAnyFramework("UE4.framework", "UnrealEngine.framework") ||
		hasFile("UE4CommandLine.txt") ||
		hasFile("uecommandline.txt")
	) {
