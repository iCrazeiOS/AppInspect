...
export async function extractDEB(
	...
	try {
		...
		const tempDir =
			destDir ?? path.join(os.tmpdir(), `appinspect-deb-${process.pid}-${Date.now()}`);
		...
		if (!alreadyExtracted) {
			...
			for (const member of members) {
				...
				fs.writeFileSync(outPath, buf.subarray(member.offset, member.offset + member.size));
			}
		...
		if (controlMember) {
			...
			try {
				...
				const controlFilePath = fs.existsSync(path.join(controlDir, "control"))
