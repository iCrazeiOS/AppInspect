...
import { EmptyState, JsonTree } from "../components";

/** Entitlements that are flagged as dangerous (red). */
const RED_PATTERNS = ["com.apple.private.", "platform-application"];

/** Entitlements that are flagged as warnings (orange). */
const ORANGE_KEYS = ["get-task-allow", "com.apple.security.cs.disable-library-validation"];
...
export function renderEntitlements(container: HTMLElement, data: any): void {
	...
	const embedded: Record<string, unknown> | null = data?.embedded ?? data?.entitlements ?? null;
	const profile: Record<string, unknown> | null = data?.profile ?? null;
	const flat: Record<string, unknown> | null =
		embedded ?? (data && typeof data === "object" && !Array.isArray(data) ? data : null);
	...
	// Source sections
	const renderSource = (label: string, obj: Record<string, unknown>): void => {
		if (hasBothSources) {
