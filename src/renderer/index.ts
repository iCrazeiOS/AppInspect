...
import { cleanupLibrariesSession, renderLibraries } from "./tabs/libraries";
...
function setLoadingPhase(phase: string, percent?: number): void {
	titlebarStatus.textContent =
		percent != null && percent > 0 ? `${phase} (${Math.round(percent)}%)` : phase;
...
async function loadTabData(tabId: string): Promise<void> {
	...
	try {
		const tabData = await window.api.getTabData(
			tab.sessionId,
			tabId as import("../shared/ipc-types").TabName
		);
		tab.loadedSectionTabs.add(tabId);
...
async function handleBinaryChange(): Promise<void> {
	...
	try {
		...
		if (matchedIdx > 0 && newArchs?.[matchedIdx]) {
			...
			const archResult = await window.api.analyseBinary(
				tab.sessionId,
				selectedIndex,
				arch.cputype,
				arch.cpusubtype
...
async function handleArchChange(): Promise<void> {
	...
	try {
		const result = await window.api.analyseBinary(
			tab.sessionId,
			tab.currentBinaryIndex,
			cpuType,
			cpuSubtype
		);
		tab.analysisResult = result;
	...
	try {
		...
		if (activeFileTabId === filePath) {
			...
			if (overviewPanel) {
				renderOverview(
					overviewPanel,
					Object.assign({}, result.overview, { hooks: (result as any).hooks }) as any
				);
			}
...
function updateExportVisibility(visible: boolean): void {
	...
	exportTabBtns.forEach((btn) => {
		btn.classList.toggle("export-active", visible);
	});
}
...
async function saveNumberSetting(
	key: "maxBundleSizeMB" | "maxFileSizeMB",
	el: HTMLInputElement
): Promise<void> {
...
optMaxBundleSize.addEventListener("change", () =>
	saveNumberSetting("maxBundleSizeMB", optMaxBundleSize)
);
