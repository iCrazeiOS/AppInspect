...
export class HexViewer {
	...
	private measureAndResize(): void {
		...
		if (this.scrollContainer && oldBytesPerRow !== best) {
			const topByte =
				Math.floor(this.scrollContainer.scrollTop / ROW_HEIGHT) * oldBytesPerRow;
	...
	private buildSearchBar(): HTMLElement {
		...
		caseBtn.addEventListener("click", () => {
			...
			caseBtn.title = this.caseSensitive
	...
	private updateLabel(): void {
		...
		this.labelEl.textContent = current ? `${base}  |  ${current.label}` : base;
	}
